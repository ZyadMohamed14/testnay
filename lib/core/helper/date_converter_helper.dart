...
class DateConverterHelper {
  ...
  DateConverterHelper({required this.configModel});

  static String formatDate(DateTime? dateTime, ConfigModel configModel, {bool isSecond = true}) {
    return isSecond
        ?  DateFormat('yyyy-MM-dd ${_timeFormatter(configModel)}:ss').format(dateTime!) :
    DateFormat('yyyy-MM-dd ${_timeFormatter(configModel)}').format(dateTime!);
  }

  static String dateToTimeOnly(DateTime dateTime, ConfigModel configModel) {
    return DateFormat(_timeFormatter(configModel)).format(dateTime);
  }
  ...
  static String localDateToIsoStringAMPM(DateTime dateTime, ConfigModel configModel) {
    return DateFormat('yyyy-MM-dd ${_timeFormatter(configModel)}').format(dateTime);
  }
  ...
  static String convertTimeToTime(String time, ConfigModel configModel) {
    return DateFormat(_timeFormatter(configModel)).format(DateFormat('HH:mm').parse(time));
  }
  ...
  static String deliveryDateAndTimeToDate(String deliveryDate, String deliveryTime, ConfigModel configModel) {
    DateTime date = DateFormat('yyyy-MM-dd').parse(deliveryDate);
    DateTime time = DateFormat('HH:mm').parse(deliveryTime);
    return '${DateFormat('dd-MMM-yyyy').format(date)} ${DateFormat(_timeFormatter(configModel)).format(time)}';
  }
  ...
  static String _timeFormatter(ConfigModel configModel) {
  ...
  static String getEstimateTime(Duration duration, BuildContext context) {
    return duration.inDays > 1
        ? '${duration.inDays} ${'day'.tr}'
        : duration.inHours > 1 ? '${duration.inHours} ${'hour'.tr}'
        : duration.inMinutes > 5 ? '${duration.inMinutes - 5} - ${duration.inMinutes} ${'min'.tr}'
        : '0 - 5 ${'min'.tr}';
  }
